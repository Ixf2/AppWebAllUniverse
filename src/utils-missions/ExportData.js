...
export function exportToPDF(data, fileName = "data.pdf") {
  ...
  const columns = [
    ...
    { header: "Agency", dataKey: "hostAgency" },
    { header: "Launch Vehicle", dataKey: "launchVehicle" },
  ...
  const rows = data.map((item) => ({
    ...
    hostAgency: cleanPDFText(item.hostAgency),
    launchVehicle: cleanPDFText(item.launchVehicle),
    discovered: cleanPDFText(item.discovered),
    distanceFromEarth: cleanPDFText(item.distanceFromEarth),
  }));
  ...
  autoTable(doc, {
    ...
    columnStyles: {
      name: { cellWidth: 40 },
      type: { cellWidth: 25 },
      hostAgency: { cellWidth: 30 },
      launchVehicle: { cellWidth: 40 },
